...
function VideoInformation(props) {
    ...
    // Create a ref for the InfoDiv
    const InfoDiv = useRef(null);

    function handleDivClick() {
        if (InfoDiv.current.classList.contains("hideComponent")) {
            InfoDiv.current.classList.replace("hideComponent", "showComponent");
        }
    }

    function handleClick() {
        if (InfoDiv.current.classList.contains("showComponent")) {
            InfoDiv.current.classList.replace("showComponent", "hideComponent");
        }
    }
    ...
    return (
        <div className="VideoInformation">
            ...
            <div ref={InfoDiv} className="InformationPopUp z-[500] bg-white text-black absolute top-0 left-0 w-[100%] hideComponent">
